...
(function () {
    ...
    function getUrl(pagetoken) {
        var pt = (typeof pagetoken === "undefined") ? "" : `&pageToken=${pagetoken}`,
            mykey = "AIzaSyAzz3pCeLDk0Nrf2g99zJWMdgdskIbGvL8",
            playListID = "PLQBQhA2KdfvM-3SHGoRbJTo5Q4Q1o3Qxv",
            URL = `https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&maxResults=50&playlistId=${playListID}&key=${mykey}${pt}`;
        return URL;
    }
    function getUrlVF(pagetoken) {
        var pt = (typeof pagetoken === "undefined") ? "" : `&pageToken=${pagetoken}`,
            mykey = "AIzaSyAzz3pCeLDk0Nrf2g99zJWMdgdskIbGvL8",
            playListID = "PLQBQhA2KdfvPLfqo1zfC0rSuAQjhZTpMK",
            URL = `https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&maxResults=50&playlistId=${playListID}&key=${mykey}${pt}`;
        return URL;
    }
            ...
            .then(data => {
                ...
                for (i = 0; i < 7; i++) {
                    ...
                    html = '<div class="col-md-3 col-sm-6 col-xs-12" style="height:300px;">' +
                        ...
                        '<h3 class="features-title font-alt" style="color:black;">' + data.items[i].snippet.title + '<h3>' +
                        '<p></p>' +
                        '</div>' +
                        '</div>';
                    
                    $(".multi-columns-row").append(html);
                }
            })
        ...
        fetch(getUrlVF(npt))
            ...
            .then(data => {
                ...
                for (i = 0; i < 9; i++) {
                    ...
                    $(".multi-columns-row-vf").append(html);
                }
            })
            .then(function (response) {
                if (response.error) {
    ...
    function responseHandler(response) {
        if (response.nextPageToken) {
            ...
            apiCallVF(response.nextPageToken);
        }
